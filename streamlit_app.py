...
if "money_balance" not in st.session_state: # ตรวจสอบ money_balance ที่นี่
    st.session_state.money_balance = 1000 # กำหนดค่าเริ่มต้นครั้งแรก
...
def record_bet_result(predicted_side, actual_result):
    ...
    # predicted_side อาจจะเป็น '?' ถ้าแนะนำให้ avoid
    if predicted_side in ['P', 'B', 'T']: # ถ้ามีการทำนาย P/B/T (ไม่ใช่ 'Avoid')
    ...
    else: # กรณีที่ระบบแนะนำให้ Avoid หรือ prediction_side เป็น '?'
        win_loss = 0 # ไม่มีได้เสียเงินจริง
        outcome = "Avoided" # เปลี่ยน outcome เพื่อให้ชัดเจนว่าไม่ได้เดิมพัน
    ...
    # เพิ่มผลลัพธ์จริงลงในประวัติของ engine ด้วย (ทำเสมอ ไม่ว่าจะเดิมพันหรือไม่)
...
st.session_state.money_balance = st.number_input(
    ...
    # ใช้ค่าใน session state โดยตรง เพราะได้กำหนดค่าเริ่มต้นไว้แล้ว
...
# ตรวจสอบว่า prediction_data มีค่าและเป็น dictionary ที่สมบูรณ์ก่อนจะเข้าถึงคีย์
if prediction_data and isinstance(prediction_data, dict) and 'recommendation' in prediction_data:
    ...
    # ถ้ามีการทำนายและแนะนำให้ "Avoid"
        ...
        with col_p_b_t[0]:
            if st.button(f"บันทึก: 🔵 P", key="no_bet_P", use_container_width=True):
                # เมื่อ Avoid ไม่มีการเดิมพัน จึงส่ง '?' เป็น predicted_side
                record_bet_result('?', 'P') 
                st.rerun()
        with col_p_b_t[1]:
    ...
    with col_p_b_t[0]:
        if st.button(f"บันทึก: 🔵 P", key="init_P", use_container_width=True):
            # ในช่วงแรกที่ยังไม่มีการทำนาย ให้ส่ง '?' เพื่อแสดงว่าไม่มีการเดิมพัน
            record_bet_result('?', 'P')
            st.rerun()
    with col_p_b_t[1]:

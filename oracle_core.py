...
class OracleBrain:
    ...
    def add_result(self, outcome: Outcome):
        ...
        if len(self.history) > 300:
            self.history.pop(0)
        if len(self.result_log) > 300:
            self.result_log.pop(0)
        if len(self.prediction_log) > 300:
    ...
    def get_module_accuracy(self) -> dict:
        ...
        for name, module in modules.items():
            win, total = 0, 0
            for i in range(6, len(self.history)):
    ...
    def get_best_recent_module(self, lookback: int = 12) -> Optional[str]:
        modules = {
            "Rule": self.rule,
            "Pattern": self.pattern,
            "Trend": self.trend,
    ...
    def predict_next(self) -> Tuple[Optional[str], Optional[str], Optional[int], Optional[str], int]:
        ...
        preds = {
            ...
            "2-2": self.two_two.predict(self.history),

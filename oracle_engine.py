...
# --- Helper Functions (common to both app and oracle engine) ---
...
def get_latest_history_string(history_list, num_results=MAX_HISTORY_FOR_ANALYSIS):
    # This function is used by app.py for display and oracle_engine for analysis
    ...
    for i in range(len(history_str) - DNA_PATTERN_LENGTH - 1, -1, -1):
        if history_str[i : i + DNA_PATTERN_LENGTH] == target_pattern:
            if (i + DNA_PATTERN_LENGTH) < len(history_str):
                ...
                relative_distance = (len(history_str) - (i + DNA_PATTERN_LENGTH)) 
...
def analyze_momentum(history_str):
    ...
    if last_streak_length >= 3:
        ...
        confidence = min(1.0, 0.60 + (last_streak_length - 3) * 0.15) 
...
# 3. Intuition (V1.14 with Dynamic Confidence for Counter Bias - retained)
...
def predict_outcome(history_list):
    ...
    # Prioritize Counter prediction if it's highly confident
